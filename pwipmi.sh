...
# Function to get AC power usage via ipmitool (from the system's BMC)
get_ac_power() {
    # Query the system's power usage through IPMI (adjust the command according to your system's capabilities)
    # Example query for power usage: ipmitool sdr type "Power Supply" | grep "Power" or check system power readings
    ac_power=$(ipmitool sdr type "Power Supply" | grep -i "Power" | awk '{print $2}')
    if [ -z "$ac_power" ]; then
        echo "0"
    else
        echo "$ac_power"
